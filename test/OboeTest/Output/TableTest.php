...
/**
 * This class tests the output of the Oboe_Table class.
 *
 * @author Philip Graham <philip@lightbox.org>
 * @package OboeTest
 * @subpackage Output
 */
class OboeTest_Output_TableTest extends PHPUnit_Framework_TestCase {

    public function testNormalBuild() {
        $tHeadRow = new Oboe_Table_Row();
        $tHeadRow->add(new Oboe_Table_Data('Column 1'));
        $tHeadRow->add(new Oboe_Table_Data('Column 2'));
        $tHeadRow->add(new Oboe_Table_Data('Column 3'));

        $tHead = new Oboe_Table_Head();
        $tHead->add($tHeadRow);

        $row1 = new Oboe_Table_Row();
        $row1->add(new Oboe_Table_Data('(1, 1)'));
        $row1->add(new Oboe_Table_Data('(1, 2)'));
        $row1->add(new Oboe_Table_Data('(1, 3)'));

        $row2 = new Oboe_Table_Row();
        $row2->add(new Oboe_Table_Data('(2, 1)'));
        $row2->add(new Oboe_Table_Data('(2, 2)'));
        $row2->add(new Oboe_Table_Data('(2, 3)'));

        $row3 = new Oboe_Table_Row();
        $row3->add(new Oboe_Table_Data('(3, 1)'));
        $row3->add(new Oboe_Table_Data('(3, 2)'));
        $row3->add(new Oboe_Table_Data('(3, 3)'));

        $tBody = new Oboe_Table_Body();
        $tBody->add(array($row1, $row2, $row3));

        $table = new Oboe_Table();
        $table->add($tHead);
        $table->add($tBody);

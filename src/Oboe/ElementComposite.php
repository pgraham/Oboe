 ...
 * @package Oboe
 */
abstract class Oboe_ElementComposite extends Oboe_ElementBase {
    ...
    protected $_objectTypes = array('Oboe_Item_Body');
    ...
    public function add($element, $push = true) {
        if (is_array($element)) {
            ...
            // This method will throw an exception if it is not a valid
            // element
     ...
     * the right type.
     *
     * @param Oboe_ElementComposite Parent element
     * @param mixed Child element
     */
    protected static function _checkElement(Oboe_ElementComposite $parent,
        $child)
    {
        // If the child is not an object make sure that non-object's can be
        // added to the composite
        if (!is_object($child)) {
            if (!$parent->_allowText) {
                throw new Oboe_Exception(
                    'Text can not be added directly to a '.get_class($parent).
                    ' object');
            }
            return;
        }
        ...
        {
            throw new Oboe_Exception('Objects can\'t be added to a '.
                get_class($parent).' element');
        }

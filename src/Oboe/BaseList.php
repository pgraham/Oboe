 ...
 * abstract element, use either Oboe_List or Oboe_Form_List depending on where
 * the list will be added to the document.
 *
 * @author Philip Graham <philip@lightbox.org>
 * @package Oboe
 */
abstract class Oboe_BaseList extends Oboe_ElementComposite {
    ...
    public function __construct($type, $id = null, $class = null) {
        if ($type != self::ORDERED && $type != self::UNORDERED) {
            throw new Oboe_Exception(
                'Oboe_List: Unrecognized list type: '. $type);
        }
     ...
     * for a Oboe_ListItem constructor.
     *

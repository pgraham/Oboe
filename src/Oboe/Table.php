 ...
 * @package Oboe
 */
class Oboe_Table extends Oboe_ElementComposite implements Oboe_Item_Body,
    Oboe_Item_Form
{
        ...
        switch ($type) {
            ...
            if ($this->_head !== null) {
                throw new Oboe_Exception(
                    'Only one head can be added to a table'
                );
            }
            $this->_head = $element;
            break;
            ...
            if ($this->_foot !== null) {
                throw new Oboe_Exception(
                    'Only one foot can be added to a table'
                );
            }
            $this->_foot = $element;
            break;
            ...
            default:
            throw new Oboe_Exception(
                'Only objects of type '.implode(',', $this->_objectTypes).
                ' can be added to a '.__CLASS__.' object');
        }
    }
    ...
    public function addHeader($header) {
        if ($this->_head === null) {
            $this->add(new Oboe_Table_Head());
        }

        $tHeadRow = $this->_head->getDefaultRow();
        $tHeader = new Oboe_Table_Data($header);
     ...
     *
     * @return Oboe_Table_Row
     */
    public function addRow() {
        $this->_checkBody();
        $row = new Oboe_Table_Row();
    ...
    public function addCell($rowIndex, $cellContents = null) {
        ...
        if ($row === null) {
            $row = new Oboe_Table_Row($rowId);
            $this->_bodies[0]->add($row);
        }

        $cell = new Oboe_Table_Data($cellContents, $id, $class);
        $row->add($cell);
        return $cell;
    }

 ...
 */
/**
 * This class encapsulates a group of Oboe_Item_Body objects.
 ...
 * @package Oboe
 */
class Oboe_Array implements Oboe_Item_Document, Oboe_Item_Body,
    ArrayAccess, Countable, Iterator
{
     ...
     *
     * @param Oboe_IPageItem the item to add to the array.
     */
    public function push(Oboe_Item_Body $item) {
        $this->_elements[] = $item;
    }
     ...
     * @param Oboe_IPageItem the item to add to the array.
     */
    public function unshift(Oboe_Item_Body $item) {
     ...
     * Oboe_Item_Body objects are accepted.
     *
    ...
    public function offsetSet($offset, $value) {
        ...
        // Validate offset
        if (!$this->_checkOffset($offset)) {
            throw new OutOfBoundsException('Oboe_Array only supports '.
                'numeric, incremental (no gaps) indexes');
        }

        // Validate value
        if (is_array($value)) {
            throw new Oboe_Exception(
                'Cannot add arrays to a Oboe_Array...yet');
        }

        if (is_object($value)) {
            if (!($value instanceof Oboe_Item_Body)) {
                throw new Oboe_Exception('Only objects of type '.
                    'Oboe_Item_Body can be added to a Oboe_Array');
            }
        } else {
            $value = (string) $value;
        }
